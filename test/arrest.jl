...
@testset "ThresholdController: HardBound step vs SmoothBound graded transition" begin
    hard = ThresholdController(metric=RawSignal(:x), bound=FixedBound(5.0), comparison=Above())
    hs = initial_controller_state(hard)
    @test level(hard, hs, 0.0, (; x=3.0), hard, hs) == 0.0
    @test level(hard, hs, 0.0, (; x=7.0), hard, hs) == 1.0
    @test register_callback(hard)

    soft = ThresholdController(metric=RawSignal(:x), bound=FixedBound(5.0), comparison=Above(), smoothing=SmoothBound(1.0e-3))
    ss = initial_controller_state(soft)
    levels = [level(soft, ss, 0.0, (; x=v), soft, ss) for v in 3.0:0.5:7.0]
...
@testset "SmoothBound with a Unitful metric/bound and matching scale" begin
    c = ThresholdController(metric=Accumulate((p, sig, m, as) -> 1.0, 0.0u"hr"), bound=FixedBound(10.0u"hr"),
                             comparison=Below(), smoothing=SmoothBound(1.0e-1), scale=1.0u"hr")
    s = initial_controller_state(c)
    levels = Float64[]
    for _ in 1:20
        push!(levels, level(c, s, 0.0, (;), c, s))
...
@testset "Direction gating (Rising/Falling)" begin
    c = ThresholdController(metric=RawSignal(:x), bound=FixedBound(5.0), comparison=Below(), direction=Falling())
    s = initial_controller_state(c)
    @test level(c, s, 0.0, (; x=(value=3.0, rate=-1.0)), c, s) == 1.0
    @test level(c, s, 0.0, (; x=(value=3.0, rate=1.0)), c, s) == 0.0
    @test level(c, s, 0.0, (; x=(value=7.0, rate=-1.0)), c, s) == 0.0
end

@testset "RawProgress has no rate: Rising/Falling direction errors clearly" begin
    c = ThresholdController(metric=RawProgress(), bound=FixedBound(0.5), comparison=Above(), direction=Rising())
    s = initial_controller_state(c)
    @test_throws "RawProgress has no notion of rate" level(c, s, 0.6, (;), c, s)
end

@testset "Window via AllController (two ThresholdControllers on RawProgress)" begin
    window = AllController(
        lo=ThresholdController(metric=RawProgress(), bound=FixedBound(0.25), comparison=Above()),
        hi=ThresholdController(metric=RawProgress(), bound=FixedBound(0.30), comparison=Below()),
    )
    s = initial_controller_state(window)
    @test level(window, s, 0.20, (;), window, s) == 0.0
    @test level(window, s, 0.27, (;), window, s) == 1.0
    @test level(window, s, 0.35, (;), window, s) == 0.0
end

@testset "Accumulate + FixedBound (cumulation), Below comparison" begin
    c = ThresholdController(metric=Accumulate((p, sig, m, as) -> 1.0, 0.0u"hr"), bound=FixedBound(10.0u"hr"), comparison=Below())
    s = initial_controller_state(c)
    for _ in 1:5
        r = controller_rate(c, s, 0.0, (;), c, s)
        s = step_state(s, r, 1.0u"hr")
    ...
    @test level(c, s, 0.0, (;), c, s) == 1.0
    for _ in 1:10
        r = controller_rate(c, s, 0.0, (;), c, s)
        s = step_state(s, r, 1.0u"hr")
    end
    @test level(c, s, 0.0, (;), c, s) == 0.0
end
...
@testset "FunctionController: fires exactly at closure zero-crossing" begin
    ...
    s = initial_controller_state(c)
    @test level(c, s, 0.0, (; x=3.0), c, s) == 0.0
    @test level(c, s, 0.0, (; x=5.0), c, s) == 1.0
    @test level(c, s, 0.0, (; x=7.0), c, s) == 1.0
end

@testset "Dual-accumulator: second stays at zero rate until first crosses its threshold" begin
    first_ctrl = ThresholdController(metric=Accumulate((p, sig, m, as) -> 1.0, 0.0), bound=FixedBound(5.0), comparison=Above())
    second_rate = (p, sig, model, arrest_state) -> level(model.controllers.first, arrest_state.first, p, sig, model, arrest_state) >= 1.0 ? 1.0 : 0.0
    second_ctrl = ThresholdController(metric=Accumulate(second_rate, 0.0), bound=FixedBound(3.0), comparison=Above())
...
@testset "Gradual breaking: SmoothBound Accumulate gives a time-continuous transition" begin
    ...
    c = ThresholdController(metric=Accumulate(temp_rate, 0.0), bound=FixedBound(1.0), comparison=Above(), smoothing=SmoothBound(0.05))
    s = initial_controller_state(c)
    levels = Float64[]
    for t in 1:60
        temp = 280.0u"K" + t * 0.3u"K"
        push!(levels, level(c, s, 0.0, (; temperature=temp), c, s))
...
@testset "Chortoicetes equivalence (discrete-loop, no callbacks)" begin
    ...
    diapause_induction = AllController(
        lo=ThresholdController(metric=RawProgress(), bound=FixedBound(diapause_lo), comparison=Above()),
        hi=ThresholdController(metric=RawProgress(), bound=FixedBound(diapause_hi), comparison=Below()),
        ...
        duration=ThresholdController(metric=Accumulate(duration_rate, 0.0), bound=FixedBound(diapause_hour_threshold), comparison=Below()),
    )
    diapause_model = ComposedArrest(induction=diapause_induction)

    window_controller(lo, hi) = AllController(
        lo=ThresholdController(metric=RawProgress(), bound=FixedBound(lo), comparison=Above()),
        hi=ThresholdController(metric=RawProgress(), bound=FixedBound(hi), comparison=Below()),
        wet=ThresholdController(metric=RawSignal(:hydration), bound=FixedBound(desiccation_tolerance), comparison=Below()),
    )
...
@testset "print_arrest_structure runs without error" begin
    c = ThresholdController(metric=RawSignal(:x), bound=FixedBound(5.0), comparison=Above())
