...
ALAT <- -23 # degrees latitude
AMINUT <- 0.7 * 60 # minutes latitude
ALONG <- 133 # degrees longitude
ALMINT <- 0.9 * 60 # minutes latitude
ALREF <- 133 # reference longitude for time zone
...
DEP <- c(0.0, 2.5, 5.0, 10.0, 15.0, 20.0, 30.0, 50.0, 100.0, 200.0) # Soil nodes (cm) - keep spacing close near the surface, last value is where it is assumed that the soil temperature is at the annual mean air temperature
...
BulkDensity <- 1.5 # soil bulk density (Mg/m3)
...
BD <- rep(1.5, 19) # soil bulk density, Mg/m3
DD <- rep(Density, 19)
...
spinup <- 0 # repeat first day ndmax times for steady periodic
...
micro <- list(
  ...
  soil      = microut$soil[1:(doynum * 24), ],
  shadsoil  = microut$shadsoil[1:(doynum * 24), ],
  soilmoist = microut$soilmoist[1:(doynum * 24), ],
  shadmoist = microut$shadmoist[1:(doynum * 24), ],
  humid     = microut$humid[1:(doynum * 24), ],
  shadhumid = microut$shadhumid[1:(doynum * 24), ],
  soilpot   = microut$soilpot[1:(doynum * 24), ],
  shadpot   = microut$shadpot[1:(doynum * 24), ],
