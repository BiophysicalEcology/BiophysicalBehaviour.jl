...
@testset "simulate_endotherm_activity_cycle" begin
    shape_pars = example_shape_pars(; mass=0.0337u"kg")
    insulation_pars = example_insulation_pars()
    conduction_pars_internal = example_conduction_pars_internal(; fat_fraction=0.05)
    radiation_pars = example_radiation_pars()
    ...
    times = (0:1:12)u"hr" .|> u"s"
    active_forcing = _diurnal_forcing(times; wind_speed=9.0u"m/s")
    resting_forcing = _diurnal_forcing(times; shade=0.9, radiation_scale=0.05, wind_speed=0.5u"m/s")
    active_temperature_max = u"K"(43.0u"°C")
    resume_temperature = u"K"(39.0u"°C")

    result = simulate_endotherm_activity_cycle(
        times, u"K"(38.0u"°C"), organism, environment_pars, active_forcing, resting_forcing;
        active_metabolic_heat_flow=mhf_bmr * 6.0, active_temperature_max, resume_temperature,
    )

    @test length(result.t) == length(result.core_temperature) == length(result.state)
    @test all(isfinite, ustrip.(u"K", result.core_temperature))
    @test any(s -> s isa Active, result.state)
    @test any(s -> s isa Resting, result.state)
    # the callback should catch the threshold crossing closely, not run away past it
    @test maximum(result.core_temperature) < active_temperature_max + 0.5u"K"
    @test result.state[1] isa Active  # starts in the active phase

    @testset "starting already above the threshold transitions immediately" begin
        result_hot = simulate_endotherm_activity_cycle(
            times, active_temperature_max + 1.0u"K", organism, environment_pars, active_forcing, resting_forcing;
            active_metabolic_heat_flow=mhf_bmr * 6.0, active_temperature_max, resume_temperature,
        )
        @test result_hot.state[1] isa Resting
    end
end
