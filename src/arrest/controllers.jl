...
# -> controller_level in [0,1]. `controller_level` is the primary quantity --
# read directly as a continuous multiplier on a process rate; hosts derive
# their own booleans from it if they need one. SmoothingStrategy picks exact
...
struct BelowBound <: AbstractComparison end
struct AboveBound <: AbstractComparison end
...
signed_gap(::BelowBound, value, bound) = bound - value
signed_gap(::AboveBound, value, bound) = value - bound

abstract type AbstractDirection end
struct AnyDirection <: AbstractDirection end
struct RisingDirection <: AbstractDirection end
struct FallingDirection <: AbstractDirection end

direction_gate(d::AbstractDirection, rate) = error("no direction_gate method for $(typeof(d))")
direction_gate(::AnyDirection, rate) = true
direction_gate(::RisingDirection, rate) = rate > zero(rate)
direction_gate(::FallingDirection, rate) = rate < zero(rate)
