...
"""
    solve_body_temperature(organism, env_vars, env_pars) → core_temperature
...
- `core_temperature`: Final body temperature (K)
...
5. **Active period thermoregulation loop** — organism traits modified in-place via `@set`:
   ...
   *Basking* (basking_temperature_min ≤ core_temperature < active_temperature_min): `orient_perpendicular`
   ...
   is in the target range or all options are exhausted.
"""
function thermoregulate(
    ::Ectotherm,
    ...
    if previous_depth > limits.depth.reference
        ...
        underground_shade_factor  = _underground_blend_factor(limits.burrow_shade_mode, limits, min_shade, max_shade, step)
        soil_temperature_at_depth = _blend(
            min_shade.soil_temperature[step, previous_depth],
            max_shade.soil_temperature[step, previous_depth],
        ...
        #   emerge_signal != 0, depth_node > 2, and no activity has occurred today yet.
        # emerge_signal > 0: soil must be warming at >= emerge_signal K/hr
        # emerge_signal < 0: soil must be cooling at >= |emerge_signal| K/hr
        stay_for_signal = false
        if !iszero(limits.emerge_signal) && previous_depth > 2 && !activity_commenced && step > 1
            soil_temperature_prev_step = _blend(
                min_shade.soil_temperature[step-1, previous_depth],
                max_shade.soil_temperature[step-1, previous_depth],
                underground_shade_factor,
            )
            # Each time step is 1 hour so the K change per step equals the K/hr rate.
            soil_delta = (soil_temperature_at_depth - soil_temperature_prev_step) / 1u"hr"
            stay_for_signal = (limits.emerge_signal > zero(limits.emerge_signal) && soil_delta < limits.emerge_signal) ||
        ...
        if soil_temperature_at_depth < limits.emerge_temperature_min || stay_for_signal
            ...
            limits = select_depth(limits, min_shade, max_shade, step, underground_shade_factor)
            env    = interpolate_environment(available_environments, step, limits, environmental_params)
            return _build_ectotherm_output(organism_current, env, environmental_params, limits, false, available_environments, underground_shade_factor)
        end
    end
...
function _build_ectotherm_output(organism, env_vars, env_pars, limits, in_active_period, available_environments, underground_shade_factor::Float64=0.0)
    e              = (environment_pars=env_pars, environment_vars=env_vars)
    is_underground = limits.depth.current > limits.depth.reference
    # When underground and solve_underground=false (default, matches NicheMapR BELOWGROUND.f):
    # set core_temperature = soil_temperature directly. The full heat balance gives
    # core_temperature > soil_temperature because metabolic heat has nowhere to go when all
    ...
    return (;
        core_temperature,
        shade            = is_underground ? (underground_shade_factor > 0.5 ? available_environments.max_shade_fraction : available_environments.min_shade_fraction) : limits.shade.current,
